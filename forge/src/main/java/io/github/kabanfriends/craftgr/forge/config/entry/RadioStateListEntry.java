package io.github.kabanfriends.craftgr.forge.config.entry;
...
public class RadioStateListEntry extends TooltipListEntry {
    ...
    public void render(PoseStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta) {
        ...
        this.buttonWidget.active = isEditable() && RadioStateListEntry.isButtonActive();
        this.buttonWidget.setX(x + entryWidth - 150);
        this.buttonWidget.setY(y);
        this.buttonWidget.setMessage(RadioStateListEntry.getButtonText());
    ...
    private static boolean isButtonActive() {
        AudioPlayerHandler handler = AudioPlayerHandler.getInstance();
        HandlerState state = handler.getState();
    ...
    private static Component getButtonText() {
        AudioPlayerHandler handler = AudioPlayerHandler.getInstance();
        HandlerState state = handler.getState();
